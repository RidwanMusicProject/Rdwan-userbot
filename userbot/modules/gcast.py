...
while 0 < 6:
    ...
    if _GCAST_BLACKLIST.status_code != 200:
        if 0 != 5:
            continue
        GCAST_BLACKLIST = [-1001267233272, -1001473548283, -100178898330, -1001788983303]
        break
...
async def gcast(event):
    ...
    if xx:
        ...
        return await edit_delete(event, "**Berikan Sebuah Pesan atau Reply Norak**")
    kk = await edit_or_reply(event, "`Globally Broadcasting Msg Kontol...`")
    er = 0
    done = 0
        ...
        if x.is_group:
            ...
            if chat not in GCAST_BLACKLIST:
                try:
                    ...
                    er += 1
    await kk.edit(
        f"**Berhasil Mengirim Pesan Ke** `{done}` **Grup, Gagal Mengirim Pesan Ke Surga** `{er}` **Grup**"
    )
...
async def gucast(event):
    ...
    if xx:
        ...
        return await edit_delete(event, "**Berikan Sebuah Pesan atau Reply Norak**")
    kk = await edit_or_reply(event, "`Globally Broadcasting Msg Jamet...`")
    er = 0
    done = 0
    async for x in event.client.iter_dialogs():
